...
const bool FIX_ASSETS = true; // Added Mark Riedl for fixed assets
...
//static bool reset_game = false; // Commented out Mark Riedl for fixed assets
...
public:
  ...
  void place_coins()
  {
    ...
    while (!rec_stack.empty()) {
      ...
      int x = r.x;
      int y = r.y;
      /* COMMENTED OUT Mark Riedl for fixed assets
      ...
      good_place &= r.y > 2;
      */
      // Added by Mark Riedl for fixed assets
      bool good_place =
    ...
    for (int y=1; y<maze->h; ++y) {
      for (int x=1; x<maze->w-1; x++) {
        ...
        if (c==FLYING_MONSTER || c==WALKING_MONSTER || c==SAW_MONSTER) {
          ...
          //int chosen_idx = randn(type_theme_idxs->size()); // Commented out Mark Riedl for fixed assets
          // added Mark Riedl for fixed assets
          int chosen_idx;
...
struct Agent {
  ...
  void reset(int spawn_n) {
    ...
    time_alive = 0;
    reward_sum = 0;
    //reset_game = false; // Commented out Mark Riedl for fixed assets
    vx = vy = spring = 0;
  ...
  void eat_coin(int x, int y)
  {
    ...
    if (is_coin(obj)) {
      ...
      if (maze->coins == 0) {
        reward += 100.0f; // Mark changed this from 10 to 100
        reward_sum += 100.0f; // Mark changed this from 10 to 100
        maze->is_terminated = true;
      } else {
       reward += 1.0f;
        reward_sum += 1.0f;
...
void stepping_thread(int n)
{
  while (1) {
...

    {
      ...
      if (game_over /*or reset_game==true*/) { // Changed Mark Riedl for fixed assets
